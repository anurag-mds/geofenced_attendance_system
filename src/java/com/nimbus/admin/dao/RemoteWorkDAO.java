...
public class RemoteWorkDAO {
    ...
    public boolean applyRemoteWork(RemoteWork remoteWork) {
        ...
        try (Connection con = DBConnection.getConnection();
            ...
            ps.setInt(1, remoteWork.getEmpId());
            ps.setDate(2, remoteWork.getStartDate());
            ps.setDate(3, remoteWork.getEndDate());
        ...
        public boolean hasOverlappingRequest(int empId, Date startDate, Date endDate) {
                String sql = "SELECT COUNT(*) FROM remote_work_approvals "
                ...
                try (Connection con = DBConnection.getConnection();
                        ...
                        System.out.println("ERROR IN CHECK REMOTE WORK OVERLAP:");
                        e.printStackTrace();
                        return true;
                }
        }
    ...
    public boolean approveRequest(
        ...
        public boolean hasOverlappingLeave(int empId, Date startDate, Date endDate) {
                String sql = "SELECT COUNT(*) FROM leave_requests "
                                + "WHERE emp_id = ? AND status IN ('SUBMITTED', 'UNDER_REVIEW', 'APPROVED') "
                                + "AND start_date <= ? AND end_date >= ?";
                try (Connection con = DBConnection.getConnection();
                         PreparedStatement ps = con.prepareStatement(sql)) {
                        ps.setInt(1, empId);
                        ...
                        System.out.println("ERROR IN CHECK LEAVE/WFH OVERLAP:");
                        e.printStackTrace();
                        return true;
                }
        }
